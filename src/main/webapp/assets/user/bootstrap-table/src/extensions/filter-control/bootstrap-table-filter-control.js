...
(function ($) {
    ...
    var copyValues = function (that) {
        ...
        header.find(searchControls).each(function () {
            that.options.valuesFilterControl.push(
                {
                    field: $(this).closest('[data-field]').data('wtf'),
                    value: $(this).val(),
            ...
            if (header.find('.date-filter-control').length > 0) {
                $.each(that.columns, function (i, column) {
                    if (column.filterControl !== undefined && column.filterControl.toLowerCase() === 'datepicker') {
                        header.find('.date-filter-control.bootstrap-table-filter-control-' + column.field).datetimepicker({"format": "yyyy-mm-dd","minView": "month","language": "zh-CN","autoclose":true,"todayBtn":false,"clearBtn":true})
    ...
    $.extend($.fn.bootstrapTable.defaults, {
        ...
        filterTemplate: {
            ...
            datepicker: function (that, field, isVisible) {
                return sprintf('<input type="text" class="form-control date-filter-control bootstrap-table-filter-control-%s" readonly="readonly" style="width: 100%; visibility: %s">', field, isVisible);
            }
        },
    ...
    BootstrapTable.prototype.onColumnSearch = function (event) {
        ...
        var $field = $(event.currentTarget).closest('[data-field]').data('wtf');
